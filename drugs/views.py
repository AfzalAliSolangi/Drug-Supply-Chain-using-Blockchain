                ...
                if key not in product_map or timestamp > product_map[key]['timestamp']:
                    product_map[key] = {
                        'product_data': {
                                        ...
                                        'image': decrypt_data(base64_to_bytes(data['products'][0]['image']))
                                        },
                        'timestamp': timestamp,
                        'email': key[0],
                        'product_code': key[1],
        ...
        if fetched_sla==Manufacturer_hash_sla:
            ...
            for product in data["products"]:
                ...
                structured_product = {
                    ...
                    "image": bytes_to_base64(encrypt_data(product["images"])),
                }
...
def pharmreqorder(request):
    ...
    if request.method == 'POST':
        ...
        if cart_items_json:
            ...
            #render a template or return an appropriate HTTP response, still to be decided
            response = rpc_connection.liststreamqueryitems('{}'.format(distributor_orders_stream), {'keys': [email_pharm]})
            json_string = json.dumps(response, indent=4) #Converts OrderedDict to JSON String
            json_string = json.loads(json_string) #Converts OrderedDict to JSON String
...
def sell_drugs(request):
    ...
    if fetched_sla==Pharmacy_hash_sla:    
        ...
        print("comp_info :" ,comp_info) 
        x = rpc_connection.subscribe('{}'.format(users_distributor_items_stream)) # Subscribing
        response = rpc_connection.liststreamkeyitems('{}'.format(users_pharmacy_items_stream), '{}'.format(email_pharm)) # Based on the manufacturer KEY the data is being fetched
        # Have a logic which fetches out items based on latest_timestamp
        print(len(response))
        if len(response) > 0:
            product_map = {} # Initialize a dictionary to store product data and timestamp for each unique key
            ...
            print(products_with_timestamp)
            return render(request, 'selldrugs.html', {'products': products_with_timestamp, 'email': email_pharm, 'company_info': comp_info})
        else:
    ...
    if request.method == 'POST':
        ...
        if fetched_sla==Pharmacy_hash_sla:            
            for cart_item in cart_items:
                    ...
                    productName = cart_item['productName']
                    timestamp = cart_item['timestamp']
                    quantity_frm_order = cart_item['quantity']
                    totalprice = cart_item['totalprice']
                    print(manu_email)
                    print(batchId)
                    print(productCode)
            ...
            if len(response) > 0:
                ...
                print(products_with_timestamp)
                return render(request, 'selldrugs.html', {'products': products_with_timestamp, 'email': email_rcvd, 'company_info': company_info})            
...
def view_solditem(request):
    ...
    if request.method == 'POST':
        ...
        if(len(json_load)>0):
            ...
            return render(request, 'viewsolditems.html', {'message': 'No Drugs Sold!', 'email': email_rcvd, 'company_info': company_info})
